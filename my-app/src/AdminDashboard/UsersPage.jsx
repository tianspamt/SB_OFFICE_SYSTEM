...
export default function UsersPage({ users, totalUsers, loading, setDeleteTarget, onEdit, onResetPassword }) {
  ...
  // Which user's mobile "more details" menu (username/email/role/actions) is
  // open — the row list only has room for ID + name at this width (see the
  // isMobile branch below), so everything else lives behind this.
      ...
      <div className={styles.tableCard}>
        ...
        <table className={styles.table}>
          <thead><tr>
            ...
            <th className={styles.th}>Role</th><th className={`${styles.th} ${styles.thSticky}`}>Action</th>
          </tr></thead>
          <tbody>
            {loading ? (
              Array.from({ length: SKELETON_ROWS }).map((_, i) => (
        ...
        {!isMobile && !loading && usersList.length === 0 && <div className={styles.empty}>No users found.</div>}
      </div>
    </>
  );
}
