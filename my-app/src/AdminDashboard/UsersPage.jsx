...
export default function UsersPage({ users, totalUsers, totalAdmins, loading, setDeleteTarget, onEdit, onResetPassword }) {
    ...
    <>
      <div className={styles.statsRow}>
        <div className={styles.statCardSolid}>
          <div className={styles.statNumberSolid}>{loading ? <span className={styles.skeletonSolid} style={{ display: "inline-block", height: 28, width: 36 }} /> : users.length}</div>
          <div className={styles.statLabelSolid}>Total Accounts</div>
        </div>
        <div className={`${styles.statCardSolid} ${styles.statCardSolidGreen}`}>
          <div className={styles.statNumberSolid}>{loading ? <span className={styles.skeletonSolid} style={{ display: "inline-block", height: 28, width: 36 }} /> : totalUsers}</div>
          <div className={styles.statLabelSolid}>Total Users</div>
        </div>
        <div className={`${styles.statCardSolid} ${styles.statCardSolidOrange}`}>
      ...
      <div className={styles.tableCard}>
        ...
        {!loading && usersList.length === 0 && <div className={styles.empty}>No users found.</div>}
      </div>
    </>
  );
}
